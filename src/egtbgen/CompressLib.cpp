...
#include "../FelicityEgtb/source/lzma/7zTypes.h"
#include "../FelicityEgtb/source/lzma/LzmaDec.h"
#include "../FelicityEgtb/source/lzma/LzmaEnc.h"
...
i64 CompressLib::compressAllBlocks(int blockSize, u8* blocktable, char *dest, const char *src, i64 slen) {
    ...
    for(int i = 0; i <= MaxGenExtraThreads; i++) {
        tmpBuf[i] = (char *)malloc(blockSize * 3 / 2);
    }

    char *p = dest;
    auto blocknum = (slen + blockSize - 1) / blockSize;
    for (int i = 0; i < blocknum; i += MaxGenExtraThreads + 1) {

        memset(compSizes, 0, sizeof(compSizes));
        std::vector<std::thread> threadVec;
        for (int j = 0; j <= MaxGenExtraThreads && i + j < blocknum; ++j) {
...
#ifdef TEST_DECOMPRESS
            int x = decompress(tmpbuf, curBlockSize, p, compSz);
...
//            int blocksz = (blocktable[i] & ~EGTB_UNCOMPRESS_BIT) - (i == 0 ? 0 : (blocktable[i - 1] & ~EGTB_UNCOMPRESS_BIT));
    ...
    for(int i = fromBlockIdx; i < toBlockIdx; i++) {
