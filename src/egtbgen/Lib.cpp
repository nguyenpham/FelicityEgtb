...
std::vector<std::string> Lib::splitString(const std::string& string, const std::string& regexString) {
    std::regex re(regexString);
    std::sregex_token_iterator first {string.begin(), string.end(), re}, last;
    return {first, last};
}

void Lib::split(const std::string &s, char delim, std::vector<std::string> &elems) {
    std::stringstream ss;
    ss.str(s);
    std::string item;
...
void Lib::replaceString(std::string& subject, const std::string& search, const std::string& replace) {
    size_t pos = 0;
...
void Lib::removeSubstrs(std::string& s, const std::string& p) {
    for (std::string::size_type n = p.length(), i = s.find(p); i != std::string::npos; i = s.find(p)) {
        s.erase(i, n);
    }
}

void Lib::toLower(char* str) {
    for(int i = 0; str[i]; ++i) {
        str[i] = tolower(str[i]);
    }
}

void Lib::toLower(std::string& str) {
    for(int i = 0; i < str.size(); ++i) {
        str[i] = tolower(str[i]);
    }
}

void Lib::toUpper(std::string& str) {
    for(int i = 0; i < str.size(); ++i) {
        str[i] = toupper(str[i]);
    }
}
...
std::string Lib::posToCoordinateString(int pos) {
...
int Lib::coordinateStringToPos(const char* s) {
...
//////////////////////////////////////////////////////////
int Lib::existFile(const char * filename)
{
    struct stat   buffer;
    return (stat (filename, &buffer) == 0);
}

std::string Lib::loadFile(const std::string& fileName) {
    std::ifstream inFile;
...
void Lib::writeTextFile(const std::string& filepath, const std::string& line)
{
    std::ofstream file;
...
std::vector<std::string> Lib::readFileToLineArray(const std::string& fileName){
    std::ifstream inFile;
    inFile.open(fileName);
...
//std::vector<std::string> Lib::getAllFileNames(const std::string& pattern) {
//    glob_t glob_result;
...
std::vector<std::string> Lib::listdir(std::string dirname) {
    std::vector<std::string> names;
    findFiles(names, dirname);
    return names;
}

void Lib::createFolder(std::string dirname) {
	if (egtb::egtbVerbose)
		std::cout << "createFolder: " << dirname << std::endl;
    CreateDirectory(dirname.c_str(),NULL);
}

i64 Lib::getFileSize(const std::string& fileName)
{
...
std::string Lib::base_name(std::string const & path) {
    return path.substr(path.find_last_of("/\\") + 1);
}

std::string Lib::getFileName(const std::string& path) {
...
void Lib::copy_file(const std::string& dest_file, const std::string& srce_file)
{
...
std::string Lib::currentTimeDate() {
...
std::string Lib::formatString(i64 num, bool format) {
    if (num < 0) {
        return "*** overflow ***";
    }
    char buf[64];
...
std::string Lib::formatSpeed(int speed) {
    const char* unit;
    if (speed > 10000000) {
    ...
    return Lib::formatString(speed) + unit;
}

std::string Lib::formatPeriod(int seconds) {
...
std::string Lib::number_percent(i64 num, i64 sz) {
    return formatString(num) + " (" + itoa(num * 100 / sz) + "%)";
}

std::string Lib::itoa(int n) {
    char buf[120];
    snprintf(buf, sizeof(buf), "%d", n);
    return buf;
}

std::string Lib::itoa(i64 n) {
    char buf[120];
    snprintf(buf, sizeof(buf), "%lld", n);
    return buf;
}

int Lib::fitBitSizeToStoreValue(int value) {
    for (int i = 1; i < 64; i++) {
        int k = 1 << i;
        if (k > value) {
            return i;
...
int Lib::encodeRL(char* buf, int size, char* toBuf) {
